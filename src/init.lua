...
local IsUserChoice = (IsTouchDevice and DevMovementMode == Enum.DevTouchMovementMode.UserChoice)
	or (DevMovementMode == Enum.DevComputerMovementMode.UserChoice)
local TouchGui = nil
...
do
	...
	local canRegisterControls = pcall(function()
...
local FFlagUserNoCameraClickToMoveSuccess, FFlagUserNoCameraClickToMoveResult = pcall(function()
	return UserSettings():IsUserFeatureEnabled("UserNoCameraClickToMove")
end)
...
function ControlState:SwitchTo(newControl)
	if ControlState.Current == newControl then
		return
	end
...
local ClickToMoveModule = FFlagUserNoCameraClickToMove
		and require(script.MasterControl:WaitForChild("ClickToMoveController"))
	or nil
...
function getTouchModule()
	...
	if not IsUserChoice then
		...
		if DynamicThumbstickAvailable and UserMovementMode == Enum.TouchMovementMode.DynamicThumbstick then
			...
			isJumpEnabled = true
		elseif
			UserMovementMode == Enum.TouchMovementMode.Default
			or UserMovementMode == Enum.TouchMovementMode.Thumbstick
		then
			module = ThumbstickModule
			isJumpEnabled = true
...
function setClickToMove()
	if
		DevMovementMode == Enum.DevTouchMovementMode.ClickToMove
		or DevMovementMode == Enum.DevComputerMovementMode.ClickToMove
		or UserMovementMode == Enum.ComputerMovementMode.ClickToMove
		or UserMovementMode == Enum.TouchMovementMode.ClickToMove
	then
...
function ControlModules.Touch:Enable()
	...
	ControlModules.Touch.LocalPlayerChangedCon = LocalPlayer:GetPropertyChangedSignal("DevTouchMovementMode")
		:connect(function()
			ControlModules.Touch:RefreshControlStyle()
		end)
	ControlModules.Touch.GameSettingsChangedCon = GameSettings:GetPropertyChangedSignal("TouchMovementMode")
		:connect(function()
			ControlModules.Touch:RefreshControlStyle()
		end)
end
function ControlModules.Touch:Disable()
	...
	if
		newModuleToDisable == ThumbstickModule
		or newModuleToDisable == DPadModule
		or newModuleToDisable == ThumbpadModule
		or newModuleToDisable == DynamicThumbstickModule
	then
...
local function getKeyboardModule()
	...
	if not IsUserChoice then
		...
		if
			UserMovementMode == Enum.ComputerMovementMode.KeyboardMouse
			or UserMovementMode == Enum.ComputerMovementMode.Default
		then
			whichModule = keyboardModule
...
local switchToInputType = function(newLastInputType)
	...
	if lastInputType == Enum.UserInputType.Touch then
		ControlState:SwitchTo(ControlModules.Touch)
	elseif
		lastInputType == Enum.UserInputType.Keyboard
