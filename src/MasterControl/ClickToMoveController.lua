...
local FFlagUserNavigationFixClickToMoveInterruptionSuccess, FFlagUserNavigationFixClickToMoveInterruptionResult = pcall(
	function()
		return UserSettings():IsUserFeatureEnabled("UserNavigationFixClickToMoveInterruption")
	end
)
local FFlagUserNavigationFixClickToMoveInterruption = FFlagUserNavigationFixClickToMoveInterruptionSuccess
	and FFlagUserNavigationFixClickToMoveInterruptionResult

local FFlagUserNavigationFixClickToMoveJumpSuccess, FFlagUserNavigationFixClickToMoveJumpResult = pcall(function()
	return UserSettings():IsUserFeatureEnabled("UserNavigationFixClickToMoveJump")
end)
local FFlagUserNavigationFixClickToMoveJump = FFlagUserNavigationFixClickToMoveJumpSuccess
	and FFlagUserNavigationFixClickToMoveJumpResult
	...
	function dataStructure:Place(position, dest)
		...
		if not self.Model.Parent then
			...
			local hitPart, hitPoint, hitNormal =
				workspace:FindPartOnRayWithIgnoreList(ray, { workspace.CurrentCamera, Player.Character })
		...
		if #this.pointList > 0 then
			...
			if FFlagUserNavigationFixClickToMoveJump then
				this.MoveToConn = this.humanoid.MoveToFinished:Connect(function(reached)
					this:OnPointReachedFixJump(reached)
				end)
			else
				this.MoveToConn = this.humanoid.MoveToFinished:Connect(function(reached)
...
local function inExtents(Extents, Position)
	if
		Position.X < (Extents.CFrame.p.X - Extents.Size.X / 2)
		or Position.X > (Extents.CFrame.p.X + Extents.Size.X / 2)
	then
		return false
	end
	if
		Position.Z < (Extents.CFrame.p.Z - Extents.Size.Z / 2)
		or Position.Z > (Extents.CFrame.p.Z + Extents.Size.Z / 2)
	then
		return false
	end
	--ignoring Y for now
	...
	if #tapPositions == 1 or goToPoint then
		if camera then
			...
			if
				hitChar
				and hitHumanoid
				and hitHumanoid.RootPart
				and (hitHumanoid.Torso.CFrame.p - torso.CFrame.p).magnitude < 7
			then
				CleanupPath()
			...
			elseif hitPt and character and not CurrentSeatPart then
				...
				if thisPather:IsValidPath() then
					...
					if hitPt then
						...
						coroutine.wrap(showQuickPopupAsync)(
							hitPt,
							foundDirectPath and "DirectWalkPopup" or "FailurePopup"
						)
					end
				end
	...
	local function OnCharacterAdded(character)
				...
				if wasInBottomRight or wasInBottomLeft then
					for otherInput, _ in pairs(FingerTouches) do
						if otherInput ~= input then
							...
							if
								otherInput.UserInputState ~= Enum.UserInputState.End
								and ((wasInBottomLeft and otherInputInRight) or (wasInBottomRight and otherInputInLeft))
							then
								if BindableEvent_OnFailStateChanged then
		...
		local function OnCharacterChildAdded(child)
			...
			if child:IsA("Humanoid") then
				...
				HumanoidSeatedConn = child.Seated:Connect(function(active, seat)
					onSeated(child, active, seat)
				end)
