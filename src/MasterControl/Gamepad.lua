...
function Gamepad:Enable()
	...
	local controlCharacterGamepad = function(actionName, inputState, inputObject)
		...
		if activateGamepad ~= inputObject.UserInputType then
			return
		end
...
function Gamepad:Disable()
	...
	if gamepadConnectedCon then
		gamepadConnectedCon:disconnect()
	end
	if gamepadDisconnectedCon then
		gamepadDisconnectedCon:disconnect()
	end
