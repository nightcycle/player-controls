...
local FFlagUserEnableDynamicThumbstickIntroSuccess, FFlagUserEnableDynamicThumbstickIntroResult = pcall(function()
	return UserSettings():IsUserFeatureEnabled("UserEnableDynamicThumbstickIntro")
end)
local FFlagUserEnableDynamicThumbstickIntro = FFlagUserEnableDynamicThumbstickIntroSuccess
	and FFlagUserEnableDynamicThumbstickIntroResult
...
function Thumbstick:Enable()
	...
	if FFlagUserEnableDynamicThumbstickIntro and Intro then
		coroutine.wrap(function()
			...
			if success and not shouldShowIntro then
				return
			end
...
function Thumbstick:Create(parentFrame)
	...
	local function fadeThumbstick(visible)
		...
		if visible then
			...
			for i = 1, #MiddleImages do
				middleImageFadeTweens[i] = TweenService:Create(
					MiddleImages[i],
			...
			for i = 1, #MiddleImages do
				middleImageFadeTweens[i] =
					TweenService:Create(MiddleImages[i], ThumbstickFadeTweenInfo, { ImageTransparency = 1 })
				middleImageFadeTweens[i]:Play()
			end
		end
	end
	...
	local function doMove(direction)
		...
		if inputAxisMagnitude < RadiusOfDeadZone then
			...
			currentMoveVector = currentMoveVector.unit
				* (1 - math.max(0, (RadiusOfMaxSpeed - currentMoveVector.magnitude) / RadiusOfMaxSpeed))
	...
	local function moveStick(pos)
		local startPos = Vector2.new(MoveTouchStartPosition.X, MoveTouchStartPosition.Y)
			- ThumbstickFrame.AbsolutePosition
	...
	ThumbstickFrame.InputBegan:connect(function(inputObject)
		...
		if IsFirstTouch then
			...
			TweenService:Create(
				EndImage,
				tweenInfo,
				{ Size = UDim2.new(0, ThumbstickSize, 0, ThumbstickSize), ImageColor3 = Color3.new(0, 0, 0) }
			):Play()
		end
		...
		local startPosVec2 = Vector2.new(
			inputObject.Position.X - ThumbstickFrame.AbsolutePosition.X,
			inputObject.Position.Y - ThumbstickFrame.AbsolutePosition.Y
		)
	...
	OnTouchMovedCn = UserInputService.TouchMoved:connect(function(inputObject, isProcessed)
		if inputObject == MoveTouchObject then
			local direction = Vector2.new(
				inputObject.Position.x - MoveTouchStartPosition.x,
				inputObject.Position.y - MoveTouchStartPosition.y
			)
	...
	OnMoveTouchEnded = function(inputObject)
		if inputObject then
			...
			if FFlagUserEnableDynamicThumbstickIntro and Intro then
				coroutine.wrap(function()
					Intro.onThumbstickMoved(direction.magnitude)
				end)()
			end
		end
	...
	OnRenderSteppedCn = RunService.RenderStepped:Connect(function(step)
		...
		elseif TweenOutAlphaStart ~= nil then
			...
			ThumbstickFrame.BackgroundTransparency = 1
				- FadeInAndOutMaxAlpha
				+ FadeInAndOutMaxAlpha * math.min(delta / fadeOutTime, 1)
		...
		if FFlagUserEnableDynamicThumbstickIntro and Intro then
			if Intro.currentState == Intro.states.MoveThumbstick then
				local startPos = StartImage.AbsolutePosition
					- ThumbstickFrame.AbsolutePosition
					+ (StartImage.AbsoluteSize * 0.5)
				local endPos = EndImage.AbsolutePosition
					- ThumbstickFrame.AbsolutePosition
	...
	local originalScreenOrientationWasLandscape = playerGui.CurrentScreenOrientation
	...
	playerGuiChangedConn = playerGui.Changed:connect(function(prop)
		if prop == "CurrentScreenOrientation" then
			if
				(
					originalScreenOrientationWasLandscape
